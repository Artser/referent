...
export async function POST(req: NextRequest) {
  try {
    ...
    const aiResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Ты аналитик статей. Выдели основные тезисы статьи на русском языке в виде пронумерованного списка. Каждый тезис должен быть кратким (1-2 предложения) и отражать ключевую мысль.',
          },
          {
            role: 'user',
            content: articleText,
    ...
    const theses = aiData.choices[0].message.content
